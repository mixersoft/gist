...
static image_list_t *create_gabor(int nscales, const int *ors, int width, int height)
{
    int i, j, fn;
    ...
    int nfilters = 0;
    for(i=0;i<nscales;i++)  nfilters+=ors[i];
    ...
    for(i = 1; i <= nscales; i++)
    {
        for(j = 1; j <= ors[i-1]; j++)
        {
            ...
            param[l][2] = 16*pow(ors[i-1], 2)/pow(32, 2);
            param[l][3] = M_PI/(ors[i-1])*(j-1);
            l++;
        }
    }
...
OutputIterator copy_n(InputIterator src, OutputIterator dst, size_t n)
{
	// It is important to avoid incrementing src any more than necessary.
	// In case of stream iterators, increment carries the side effect of
	// consuming a token.
	if (!n)
		return dst;
	*dst = *src;
	...
	)
{
	...
	image_t * tempImg = image_new(w, h);

	copy_n(image.data, tempImg->data, w * h);

	float * descRaw(bw_gist(tempImg, blockCount, a, b, c));

	descriptor.resize(blockCount * blockCount * (a + b + c));
	copy_n(descRaw, descriptor.begin(), descriptor.size());

	image_delete(tempImg);
	free(descRaw);
}
...
/*
array<float> ^ Gist::Algorithm::GetColorDescriptor
	...
	)
{
	...
	color_image_t * tempImg = color_image_new(w, h);

	for (int y(0); y != h; ++y)
	{
		for (int x(0); x != w; ++x)
		{
			tempImg->c1[y * w + x] = static_cast<float>(image->Data[y, x, 2]);
			tempImg->c2[y * w + x] = static_cast<float>(image->Data[y, x, 1]);
			tempImg->c3[y * w + x] = static_cast<float>(image->Data[y, x, 0]);
		}
	}

	float * descRaw(color_gist(tempImg, blockCount, a, b, c));

	array<float> ^ desc = gcnew array<float>(3 * blockCount * blockCount * (a + b + c));
